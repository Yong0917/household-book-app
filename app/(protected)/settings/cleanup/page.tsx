// 가계부 정리 페이지 (Server Component)
import Link from "next/link";
import { ArrowLeft, Lock } from "lucide-react";
import { CleanupTool } from "@/components/settings/CleanupTool";
...
export default async function CleanupPage() {
  ...
  const isAdmin = data?.claims?.sub === process.env.ADMIN_USER_ID;
    ...
    <>
      ...
      <header className="sticky top-0 bg-background border-b z-10 px-4" style={{ paddingTop: "env(safe-area-inset-top)" }}>
        <div className="h-14 flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold">가계부 정리</h1>
        </div>
      </header>
      ...
      {isGuest ? (
        ...
        <CleanupTool initialCategories={await getCategories()} isAdmin={isAdmin} />
      )}
    </>
  );
}
